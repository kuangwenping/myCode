...
define('common/main',['bui/menu','bui/tab'],function(require) {
    ...
    var CLS_SELECTE = 'dl-selected',//选中的模块样式
        ...
        CLS_HOVER = 'dl-hover',
        CLS_ITEM = 'nav-item',
        CLS_LEFT_SLIB = 'dl-second-slib', //折叠
    ...
    function tabNav(moduleId,tabConfig,menuConfig,collapsed,homePage){    
      ...
      menu.on('menuclick',function(ev){      
        var item = ev.item;
        if(item){
          _self.tab.addTab({id: item.get('id'),
           title: item.get('text'), href: item.get('href'),
           closeable : item.get('closeable')},true);
        }
      });
    ...
    BUI.augment(mainPage,{
      ...
      _initEvent : function(){
        ...
        //item= _self._getModuleConfig(id),
        modules = _self.get('modules');
        modules.menu.tab.on('closed',function(ev){                                                   
          var item = ev.target;
          var id = item.get('id');  
  
                  
            // _self.clonnnn=function(cb){
            //     alert();
            //     if(cb){
            //       cb(111);
            //     }
            // }      
            //console.log(_self.clonnnn);
        });
