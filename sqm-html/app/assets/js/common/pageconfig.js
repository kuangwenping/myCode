...
seajs.use(['bui/tab', 'bui/mask'], function (Tab) {
    ...
    $('#disBtn').on('click', function () {
        tab.disable();
        var isDisable = tab.get('disabled');
        if (isDisable) {
            tab.on('itemclick', function (e) {
                //禁用逻辑
                return
            });
        }
    ...
    new Tab.TabPanel({
        ...
        tpl: '<a class="float-r mar-t10 mar-r20 more-btn">更多>></a>'
    });

    new Tab.TabPanel({
        render: '#taskTab',
        forceFit: true,
        panelContainer: '#pane2', //如果内部有容器，那么会跟标签项一一对应，如果没有会自动生成    
        autoRender: true,
        children: [{
                //title:'流程代办',
                value: '2',
                selected: true,
                tpl: '<span class="bui-tab-item-text"><span>流程代办</span><i class="hint-green">89</i></span>',
                elCls: 'LCDB-item'
            },
            {
                title: '措施落实',
...
seajs.use('bui/select', function (Select) {
    ...
    //select1.updateContent();
    select1.set('items', items1);
    select1.render()
...
BUI.use(['bui/grid', 'bui/data', 'bui/toolbar'], function (Grid, Data, Toolbar) {
    ...
    // 数据字典（序号）
    var dictionariesEl = new Grid.Grid({
        ...
        new Grid.Grid({
            ...
            plugins: [checkListediting, Grid.Plugins.CheckSelection, Grid.Plugins.ColumnResize] // 插件形式引入多选
        });
...
seajs.use('czy/check', function (check) {
    $(function () {
        ...
        var che = new check.Select({
            pattern: 'radio', //单选 radio   多选check
            ...
            //操作栏 （根据需求）
            callBack: function ($currEl, itemData) {
                console.log($currEl, itemData);
            }
        });
...
seajs.use('czy/check', function (check) {
    $(function () {
        var items = [{
            text: '主动报告', //必选
            id: 't1', //id （根据需求）
            value: 'name1', //设置value值 （根据需求）
            elCls: 'select-item', //(不需要改动)
            checked: true, //默认值 （根据需求）
            beforeTpl: '<i class="icon-radio"></i>', //添加图标 （不需要改动）
            disabled: true,
        }, {
            text: '主动报告',
            id: 2, //id
            ...
            text: '主动报告',
            id: 4, //id
            value: 'name3', //设置value值
            beforeTpl: '<i class="icon-radio"></i>', //添加图标 （不需要改动）
        }]

        var kk = new check.Select({
            pattern: 'radio', //单选   多选
            rendEl: '#radioSelect1', //包裹层
            items: items, //选项配置
            elCls: 'clear', //选项父元素 (不需要改动)
            name: 'cc', //name值 （根据需求）
            elName: 'li', //元素名称 （可不改动）
            autoRender: true, //自动绘制 （根据需求）
        });
...
seajs.use('czy/check', function (check) {
    $(function () {
        var items = [{
            ...
            elCls: '', //(不需要改动)
            checked: true, //默认值 （根据需求）
            beforeTpl: '<i class="czy-checkbox"></i>', //添加图标 （不需要改动）
            disabled: true,
        }, {
            text: '主动报告',
            id: 2, //id
...
BUI.use(['bui/toolbar', 'bui/data'], function (Toolbar, Data) {
    ...
    new NumerPBar({
        ...
        showRangeCount: 1, //当前页，2边的按钮数目，默认为1
        prevText: '上一页',
        nextText: '下一页',
    });
...
seajs.use('czy/searchSelect', function (select) {
    ...
    var ttt = new select.SearchSelect({
    ...
    console.log(ttt.deploy.datas);
});

seajs.use('czy/dateCalendar', function (dateCalendar) {
    var items = [{
            date: "2017-09-10",
            data: {
                start: 6,
    ...
    var workCalendar = new dateCalendar.Calendar({
        rendEl: '#datapickerThunk',
        autoRender: true,
        items: items,
        //日期移入回调
        stateCb: function () {
...
seajs.use('czy/sliderBanner', function (sliderBanner) {
    items = [{
            href: '..',
