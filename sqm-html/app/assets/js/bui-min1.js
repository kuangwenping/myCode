   ...
   *  });
    ...
    define('czy/slideThunk', ['bui/common'], function (require) {
        ...
        slideThunk.display.prototype = {
            init: function () {
                var _this = this;
                $(_this.btn).each(function (i, item) {
                    $item = $(item);
                    _this.moveTo($item);
                });
            ...
            moveTo: function ($currEl){
                var _this = this;    
                if($currEl.attr('data-flg') === undefined){                
                    $currEl.attr('data-flg',1);
                };
    
                var flg = parseInt($currEl.attr('data-flg'));
    
                var length = 0;
                var $moverObj = $currEl.siblings(_this.moveEl);
                            
                if (flg) {                                
                    length = _this.moveLength(flg, $moverObj);
                    $currEl.attr('data-flg', 0);
                } else {                            
                    $currEl.attr('data-flg', 1);                
                    length = _this.moveLength(flg, $moverObj);                
                }
    ...
    define('czy/check', ['bui/common'], function (require) {
        ...
        check.Select.prototype = {
            ...
            unitRender: function (parame) {
                var unitEl = '';
                if (parame.elData && parame.elData.length) {
            ...
            event: function () {
                ...
                this.deploy.itemEl.on('click', function (e) {
                    ...
                    var idx = $(this).index();
                    _this._setSelected(this);
                    _this.callBack && _this.callBack($(this), _this.deploy.items[idx]);
                })
            ...
            _setSelected: function (currEl) {
                if (this.deploy.pattern === 'check') {
                    this.checkSelect(currEl);
                } else if (this.deploy.pattern === 'radio') {
                    this.radioSelect(currEl);
                }
            },
            ...
            disableSelect: function () {
                this.deploy.itemEl.each(function (i, item) {
                    $(item).addClass('czy-disabled');
                });
            },
            //启用
            startSelect: function () {
                this.deploy.itemEl.each(function (i, item) {
                    $(item).removeClass('czy-disabled');
                });
            },
            //设置选中
            setSelected: function (val) {
                ...
                $(this.deploy.items).each(function (i, item) {
                    if (item.value === val) {
                        var pattern = _this.deploy.pattern;
                        var currEl = _this.deploy.itemEl[i];
                        if ($(currEl).hasClass('czy-disabled')) {
                            return
                        }
                        if (pattern === 'check') {
                            $(currEl).attr('data-check', 1);
                            ...
                            _this._setSelected(_this.deploy.itemEl[i]);
                        } else {
                            console.log.error("pattern值配置有误");
                        }
                    }
                });
            },
...
define('czy/searchSelect', ['bui/common'], function (require) {
    ...
    czySelect.SearchSelect.prototype = {
        ...
        event: function () {
            ...
            _this.deploy.inputEls.on('keyup', function (e) {                
                //删除按钮
                if(_this.deploy.items.length && _this.deploy.inputMode ){                    
                    if(e.which === 8){
                        console.log($(this).index());
                        _this.delSelected(_this.deploy.items.length-1);                        
                        if(_this.deploy.inputEls.val()){
                            _this.deploy.inputEls[0].focus();
                        }                        
                        return
                    }
                }
            ...
            //input获取焦点事件
            // _this.deploy.inputEls.focus(function(){
            //     if (_this.deploy.trigger) {
            //         //_this.selectRender($(this), val);
        ...
        selectListRender: function ($currEl, result){
            ...
            var $selectListEl = $('.input-search-select-list');
            ...
            if (!$selectListEl.length) {
                //注入单元包裹层
                var ulEl = $('<ul class="input-search-select-list"></ul>');
                ulEl.append(tpl);
                //注入父级元素
                $('body').append(ulEl);
            } else {
                $selectListEl.empty();
                $selectListEl.hide();
                tpl && $selectListEl.append(tpl) && $selectListEl.show();
            }
    ...
    /**
     * @fileOverview 工作台时间处理
     * 
     */
    define('czy/dateCalendar', ['bui/common'], function (require) {
        var BUI = require('bui/common'),
            dateCalendar = BUI.namespace('dateCalendar');
    
        dateCalendar.Calendar = function (deploy) {
            this.deploy = deploy;
            this.deploy.monthData = null;
            this.deploy.hintBtn = null;
            this.init();
        };
    
        dateCalendar.Calendar.prototype = {
            //初始化
            init: function () {
                //获取时间数据
                this.deploy.monthData = this._getmonthData();
                //自动渲染
                this.deploy.autoRender && this.render();
            },
            ...
            //绘制
            render: function () {
                if ($(this.deploy.rendEl).children().length) {
                    $(this.deploy.rendEl).empty();
                }
                var _this = this;
                var monthData = this.deploy.monthData;
                var testDatas = this.deploy.items;
    
                var html = '<div class="month-select">' +
                    '<a href="#" class="czy-icon line-arrows-left" id="J-prevMonth-btn"></a> ' +
                    '<h4 id="J-displyMonth" class="curr-month">' + monthData[0].currYear + '年' + monthData[0].currMonth + '月</h4>' +
                    '<a href="#" class="czy-icon line-arrows-right" id="J-nextMonth-btn"></a>' +
                    '</div>' +
                    '<ul class="date-list">';
    
                $(monthData).each(function (i, item) {
                    if (item.month === item.currMonth) {
                        html += '<li class="date-item">' +
                            '<span class="date-text">' + item.showdate + '</span>';
                        $(testDatas).each(function (inx, data) {
                            var date = new Date(data.date);
                            var year = date.getFullYear();
                            var month = date.getMonth() + 1;
                            var day = date.getDate();
                            if (year === item.currYear && month === item.currMonth && day === item.showdate) {
                                var data = data.data;
                                html += '<span class="hint-orange czy-none">' + data.total + '</span> ' +
                                    '<div class="num-list czy-none">' +
                                    '<span class="num-item left-top flow-num">' + data.LCDB + '</span>' +
                                    '<span class="num-item right-top workable-num">' + data.CSLS + '</span>' +
                                    '<span class="num-item left-bottom verify-num">' + data.CSYZ + '</span>' +
                                    '<span class="num-item right-bottom allot-num">' + data.YZFP + '</span>' +
                                    '</div>' +
    
                                    '<div class="footer-lab czy-none">' +
                                    '<span class="ellipsis">' + data.start + '</span>' +
                                    '<span class="special-lab ellipsis">' + data.end + '</span>' +
                                    '</div>';
                            }
                        });
                        html += '</li>';
                ...
                html += '</ul>';
                $(this.deploy.rendEl).append(html);
                _this.elInit();
                _this.eventInit();
            },
            ...
            //元素初始化
            elInit: function () {
                this.deploy.$prevMonthBtn = $('#J-prevMonth-btn');
                this.deploy.$nextMonthBtn = $('#J-nextMonth-btn');
                this.deploy.$dateEls = $('.date-item');
            },
    
            //事件初始化
            eventInit: function () {
                var _this = this;
                this.deploy.$prevMonthBtn.on('click', function (e) {
                    e.preventDefault();
                    var currDate = _this.countCurrDate(-1);
                    _this.deploy.monthData = _this._getmonthData(currDate.year, currDate.month);
                    _this.render();
                });
    
                this.deploy.$nextMonthBtn.on('click', function (e) {
                    e.preventDefault();
                    var currDate = _this.countCurrDate(1);
                    _this.deploy.monthData = _this._getmonthData(currDate.year, currDate.month);
                    _this.render();
                ...
                //鼠标移入日期
                this.deploy.$dateEls.on('mouseenter', function (e) {
                    e.stopPropagation()
                    var $childrens = $(this).children();
                    var $lab = $(this).find('.footer-lab');
                    _this.deploy.hintBtn = $(this).find('.hint-orange');
    
                    $(this).addClass('selected');
                    _this.deploy.hintBtn.removeClass('czy-none');
                    $lab.removeClass('czy-none');
    
    
                });
    
                //提示点击
                $('.hint-orange').on('click', function (e) {
                    var numPar = $(this).siblings('.num-list');
                    if (numPar.hasClass('czy-none')) {
                        numPar.removeClass('czy-none');
                        var numItems = numPar.children();
    
                        numItems.on('click', function (e) {
                            $(this).parent().addClass('czy-none');
                        });
    
                    } else {
                        numPar.addClass('czy-none');
                    }
                });
    
    
                this.deploy.$dateEls.on('mouseleave', function (e) {
                    var $childrens = $(this).find('.date-text').nextAll();
                    //console.log($(this).find('.date-text'));
                    if ($childrens.length) {
                        $childrens.each(function (i, item) {
                            $(item).addClass('czy-none');
                        });
                    }
                    ...
                    // _this.deploy.hintBtn.addClass('czy-none');
                    // $lab.addClass('czy-none');
    
                });
    
    
            },
            countCurrDate: function (limit) {
                var month = this.deploy.monthData[0].currMonth + limit;
                var year = this.deploy.monthData[0].currYear;
            ...
            _getmonthData: function (year, month) {
                ...
                firstDayweekDay === 0 ? firstDayweekDay = 1 : firstDayweekDay;
                ...
                for (var i = 0; i < (7 * 6); i++) {
                    var date = i + 1 - preMonthDayCount; //当前日期
                    var showDate = date; //显示日期
                    var thisMonth = month; //显示月份
                    var timestamp = new Date(year, thisMonth, showDate).getTime();
     ...
     * 轮播
     */
    define('czy/sliderBanner', ['bui/common'], function (require) {
        var BUI = require('bui/common'),
            sliderBanner = BUI.namespace('sliderBanner');
        sliderBanner.Slider = function (deploy) {
        ...
        sliderBanner.Slider.prototype = {
            ...
            render: function () {
                var _this = this;
                var items = this.deploy.items;
                var $renderEl = $(this.deploy.renderEl);
    
                var sliderEl = $('<ul class="slide-list"></ul>');
    
                if (this.deploy.anchor) {
                    anchorEl = $('<ul class="anchor-list"></ul>');
                }
    
                //注入slider单元
                $(items).each(function (i, item) {
                    var sliderItemEl = $('<li class="slide-item"></li>');
    
                    //图片
                    var imgEl = $('<img/>');
                    item.alt && imgEl.attr('alt', item.alt);
                    item.imgsrc && imgEl.attr('src', item.imgsrc);
    
                    //文字
                    if (item.text) {
                        var textEl = $('<p class="slide-text"></p>');
                        textEl.text(item.text);
                    }
    
                    //链接
                    if (item.href) {
                        var linkEl = $('<a class="slide-link"></a>');
                        linkEl.attr('href', item.href);
                        linkEl.append(imgEl);
                        linkEl.append(textEl);
                        sliderItemEl.append(linkEl);
                    } else {
                        sliderItemEl.append(imgEl);
                        sliderItemEl.append(textEl);
                    }
    
                    //锚点
                    if (_this.deploy.anchor) {
                        var anchorItemEl = $('<li class="anchor-item"></li>');
                        anchorEl.append(anchorItemEl);
                    }
    
                    //当前显示
                    if (_this.deploy.start === item.id) {
                        _this.deploy.active = i;
                        _this.deploy.anchor && anchorItemEl.addClass('anchor-item-active');
                        sliderItemEl.addClass('slide-item-active');
                    }
                    sliderEl.append(sliderItemEl);
                });
    
                $renderEl.append(sliderEl);
    
                if (this.deploy.anchor) {
                    $renderEl.append(anchorEl);
                }
    
                this.elInit();
                if (this.deploy.autoPlay) {
                    this.moverTo();
                }
            ...
            elInit: function () {
                this.deploy.parEl = $(this.deploy.renderEl); // 外层父元素            
                this.deploy.sliderEl = this.deploy.parEl.children('.slide-list');
                this.deploy.sliderItemEls = this.deploy.sliderEl.children();
                this.deploy.anchorEl = this.deploy.parEl.children('.anchor-list');
                this.deploy.anchorItemEls = this.deploy.anchorEl.children();
            },
    
            moverTo: function () {
                var _this = this;
                this.deploy.timer = setTimeout(function () {
                    _this.slider();
                    _this.deploy.sliderCB && typeof _this.deploy.sliderCB === 'function' && _this.deploy.sliderCB();
                    _this.moverTo();
                }, this.deploy.speed);
            },
    
            stopMover: function () {
                clearTimeout(this.deploy.timer);
            },
    
            slider: function () {
                var _this = this;
                var totalW = this.deploy.sliderEl.width();
                var unitW = totalW / this.deploy.items.length;
                var sliderItemEls = this.deploy.sliderItemEls;
    
                // if(_this.deploy.active >= _this.deploy.items.length-1){
                //     _this.deploy.active = 0;
                // }            
                // _this.deploy.active+=1;                                    
                //this.deploy.moverLength = $(sliderItemEls[_this.deploy.active]).position().left*this.deploy.wheel;
    
                this.deploy.moverLength += unitW * this.deploy.wheel;
                var absMoverLength = Math.abs(this.deploy.moverLength);
    
                if (absMoverLength >= totalW) {
                    this.deploy.moverLength = 0;
                }
    
                this.deploy.sliderEl.css({
                    marginLeft: _this.deploy.moverLength + 'px',
                    transition: 'all 1s'
                });
            },
            ...
            eventInit: function () {
                var _this = this;
                this.deploy.parEl.on('mouseenter', function () {
                    _this.stopMover();
                });
    
                this.deploy.parEl.on('mouseleave', function () {
                    _this.moverTo();
                });
            }
        }
 ...
 * 滚动定位
 */
define('czy/sideCatalog', ['bui/common'], function (require) {
    var BUI = require('bui/common'),
        sideCatalog = BUI.namespace('sideCatalog');
    var SideToolbar = function (wrapper, options) {
        var defaults = {width: 280, height: 260, speed: 300, right: 10, bottom: 10};
        this.settings = $.extend(defaults, options);
        this.mouseUpDown = true;
        this.timer = null;
        this.eleOpts = {wrapper: $(wrapper)};
        this.initView();
        this.bindEvents();
    };
    SideToolbar.prototype = {
        initView: function () {
            var that = this;
            that.eleOpts.main = $('<div class="main"></div>').appendTo(that.eleOpts.wrapper);
            that.eleOpts.wrapper.css({
                width: that.settings.width + 20,
                height: that.settings.height + 20,
                right: that.settings.right,
                bottom: that.settings.bottom
            });
            that.eleOpts.wrapper.fadeIn(that.settings.speed);
            that.eleOpts.main.css({width: that.settings.width, height: that.settings.height});
            that.eleOpts.sidebar = $('<div class="sidebar"></div>').appendTo(that.eleOpts.main);
            that.eleOpts.catalog = $('<div class="catalog"></div>').appendTo(that.eleOpts.main);
            that.eleOpts.catalog_dl = $('<dl></dl>').appendTo(that.eleOpts.catalog);
            for (var i = 0, l = that.settings.cataList.length; i < l; i++) {
                var cata = that.settings.cataList[i];
                html = [];
                html.push('<dd class="catalog-item">');
                if (i === 0) {
                    html.push('<span class="over_top"></span>');
                }
                if (i === that.settings.cataList.length - 1 && i > 0) {
                    html.push('<span class="over_bottom"></span>')
                }
                html.push('     <div title="' + cata.title + '" onclick="return false;">' + cata.title + '</div>');
                html.push('     <span class="dot"></span>');
                html.push('</dd>');
                var cEle = $(html.join('')).appendTo(that.eleOpts.catalog_dl);
                i === 0 && cEle.addClass('active');
                cata.toEle = $(cata.anchor);
                cata.cEle = cEle;
                cEle.data('goTop', cata.toEle.offset().top).click(function () {
                    $('body, html').animate({scrollTop: $(this).data('goTop')}, 300, 'linear');//不能写$('document')???
                })
            }
            var absHeight = $(that.settings.cataList[that.settings.cataList.length - 1].anchor).offset().top - $(that.settings.cataList[0].anchor).offset().top,
                slidHeight = that.settings.height - that.settings.cataList[0].cEle.outerHeight() - that.settings.cataList[that.settings.cataList.length - 1].cEle.outerHeight(),
                topTotal = 0;
            for (var i = 1, l = that.settings.cataList.length; i < l; i++) {
                var cata = that.settings.cataList[i];
                if (i === that.settings.cataList.length - 1) {
                    cata.cEle.css('margin-top', slidHeight - topTotal);
                } else {
                    var top = Math.floor(((cata.toEle.offset().top - that.settings.cataList[i - 1].toEle.offset().top) / absHeight) * slidHeight - cata.cEle.outerHeight());
                    top = (top < 0 ? 5 : top);
                    topTotal += top + cata.cEle.outerHeight();
                    cata.cEle.css('margin-top', top);
                }
            }
        },
        bindEvents: function () {
            ...
            $(window).scroll(function () {
                ...
                var isFind = false;
                for (var i = 0, len = that.settings.cataList.length; i < len; i++) {
                    var ele = that.settings.cataList[i];
                    ele.cEle.removeClass('active');
                    if ((scrollTop <= ele.toEle.offset().top + ele.toEle.height()) && !isFind) {
                        isFind = true;
                        ele.cEle.addClass('active');
                    }
                }
                if (!isFind) {
                    that.settings.cataList[that.settings.cataList.length - 1].cEle.addClass('active');
                }
            });
        }
    };
    sideCatalog.init = function (wrapper, options) {
        new SideToolbar(wrapper, options);
    };
    return sideCatalog;
});
 ...
 * 级联下拉
 */
define('czy/cascader', ['bui/common'], function (require) {
    var BUI = require('bui/common'),
        cascader = BUI.namespace('cascader');
    cascader.Cascader = function (opts) {
        this.opts = {
            $ele: '#J_Select',
            valueSymbol: ','/**内容分隔符**/,
            gloupSymbol: '/'/**组分隔符**/,
        ...
        this.singleJson = {};//单个
        this.eleOpts = {};//对象
        this.checkJson = {};//已经选中的数据
        for (var i = 0, item; item = this.opts.data[i++];) {
            this.groupJson[item.pid] || (this.groupJson[item.pid] = []);
            this.groupJson[item.pid].push(item);
            this.singleJson[item.id] = item;
        }
        for (var key in this.singleJson) {
            var item = this.singleJson[key];
            while (true) {
                if (this.singleJson[item.pid]) {
                    this.singleJson[key].pObj || ( this.singleJson[key].pObj = [this.singleJson[key]]);
                    this.singleJson[key].pName || ( this.singleJson[key].pName = [this.singleJson[key].name]);
                    item = this.singleJson[item.pid];
                    this.singleJson[key].pObj.unshift(item);
                    this.singleJson[key].pName.unshift(item.name);
                } else {
                    break;
                }
            }
    ...
    cascader.Cascader.prototype = {
        /**
         * 初始化
         */
        init: function () {
            var that = this;
            that.eleOpts.container = $(that.opts.$ele);
            that.eleOpts.input = $('<input type="text" readonly="readonly" value="请选择"/>').appendTo(that.eleOpts.container);
            that.eleOpts.main = $('<div class="ui-cascader-menus visibility"></div>').appendTo($(document.body));
            that.eleOpts.head = $('<div class="head"></div>').appendTo(that.eleOpts.main);
            that.eleOpts.search = $('<input type="text" placeholder="请输入查询条件"/>').keyup(function (event) {
            ...
            }).appendTo(that.eleOpts.head);
            that.eleOpts.search.blur(function (event) {
                $(this).keyup(event);
            });
            that.eleOpts.search.click(function (event) {
                event.stopPropagation();
            });
            that.eleOpts.body = $('<div class="main"></div>').appendTo(that.eleOpts.main);
            that.eleOpts.container.click(function (event) {
                if (that.opts.disabled) return false;
                that.eleOpts.search.val('');
                event.stopPropagation();
                var offset = $(this).offset();
                that.eleOpts.main.css({
                    left: offset.left,
                    top: offset.top + $(this).outerHeight()
                }).removeClass('visibility');
                that.create(0);
                $(document.body).one('click', function () {
                    that.eleOpts.main.hasClass('visibility') || that.eleOpts.main.addClass('visibility');
                })
            });
        },
        /**
         ...
         */
        disabled: function () {
            this.opts.disabled = !this.opts.disabled;
        },
        /**
         * 搜索处理
         * @param sKey 搜索关键字
        ...
        search: function (sKey) {
            var that = this;
            that.eleOpts.body.html('');
            var block = $('<ul class="block search" style="border: none"></ul>').click(function (event) {
                event.stopPropagation();
            }).appendTo(that.eleOpts.body);
            var isFind = false;
            for (var key in that.singleJson) {
                var item = that.singleJson[key];
                if (!that.groupJson[item.id] && item.pName && item.pName.join('').indexOf(sKey) > -1) {
                    isFind = true;
                    var html = item.pName.join(that.opts.gloupSymbol);
                    var regex = new RegExp(sKey, 'gi');
                    html = html.replace(regex, function (val) {
                        return '<b>' + val + '</b>';
                    });
                    var li = $('<li class="item check"><i></i><span>' + html + '</span></li>').data('source', item).click(function (event) {
                        event.stopPropagation();
                        var source = $(this).data('source');
                        if (that.checkJson[source.id]) {
                            $(this).removeClass('active');
                            delete that.checkJson[source.id];
                        } else {
                            $(this).addClass('active');
                            that.checkJson[source.id] = source;
                        }
                        that.changeValue();
                    }).appendTo(block);
                    //是否已经选中过
                    that.checkJson[item.id] && li.addClass('active');
                }
            }
            if (!isFind) {
                $('<li class="item no-data">无匹配数据</li>').appendTo(block);
            }
        },
        /**
         * 获取选择的内容
        ...
        /**
         * 设置默认值
         * @param idArr  ID数组 同时ID必须为最后一级
         */
        setValue: function (idArr) {
            var that = this;
            for (var i = 0, id; id = idArr[i++];) {
                if (that.groupJson[id]) {
                    continue;
                } else {
                    that.checkJson[id] = that.singleJson[id];
                }
            }
            that.changeValue();
        },
        ...
        changeValue: function () {
            var strArr = [], that = this;
            for (var key in that.checkJson) {
                strArr.push(that.checkJson[key].pName.join(that.opts.gloupSymbol));
            }
            that.eleOpts.input.val(strArr.join(that.opts.valueSymbol));
            that.eleOpts.input.attr('title', strArr.join(that.opts.valueSymbol));
        },
        /**
         * 下拉创建
         * @param pid
         */
        create: function (pid) {
            var that = this, sourceJson = that.groupJson[pid];
            pid === 0 && that.eleOpts.body.html('');
            if (sourceJson) {
                var block = $('<ul class="block"></ul>').click(function (event) {
                    event.stopPropagation();
                }).data('pid', pid).appendTo(that.eleOpts.body);
                if (pid === 0) {
                    block.css('border', 'none');
                }
                for (var i = 0, item; item = sourceJson[i++];) {
                    var li = $('<li class="item"><i></i><span>' + item.name + '</span></li>').data('source', item).click(function (event) {
                        event.stopPropagation();
                        var source = $(this).data('source');
                        if ($(this).hasClass('check')) {
                            ...
                            var nPid = $(this).parent().next().data('pid');
                            if (nPid !== source.pid) {
                                var isFind = false;
                                block.parent().children().each(function () {
                                    if (isFind) {
                                        $(this).remove();
                                    } else if ($(this).data('pid') == pid) {
                                        isFind = true;
                                    }
                                });
                                $(this).hasClass('active') || $(this).addClass('active');
                                that.create(source.id);
                            }
                        }
                    }).appendTo(block);
                    //不存在子集的话为最后一级加check标识
                    that.groupJson[item.id] || li.addClass('check');
                    //是否已经选中过
                    that.checkJson[item.id] && li.addClass('active');
                }
            }
...
(function () {
    ...
    BUI.use(["bui/common", "bui/data", "bui/list", "bui/picker", "bui/menu", "bui/toolbar",
        "bui/progressbar", "bui/cookie", "bui/form", "bui/mask", "bui/select", "bui/tab", "bui/calendar", "bui/overlay",
        "bui/editor", "bui/grid", "bui/tree", "bui/tooltip",
        'bui/uploader', 'czy/shrink', 'czy/slideThunk', 'czy/check', 'czy/menuSlide', 'czy/searchSelect',
        'czy/dateCalendar', 'czy/sliderBanner','czy/sideCatalog', 'czy/cascader'
    ]);
})();
